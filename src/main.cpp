			...
			if (s != "") {
				...
				if (event == "telemetry") {
					...
					*/
					...
					auto x_wp_car = Eigen::VectorXd(len);
					auto y_wp_car = Eigen::VectorXd(len);
					...
					// Define states in vehicle coordinates: x, y, psi, v, cte, e_psi
					Eigen::VectorXd state(6);
					state << 0.0, 0.0, 0.0, v, cte, e_psi;

					// Get optimal trajectory from MPC
					auto results = mpc.Solve(state, coeffs);
					double steer_value = -results[0] / 0.436332;
					...
					for (unsigned int i = 0; i < x_wp_car.size(); ++i) {
						next_x_vals.push_back(x_wp_car(i));
						next_y_vals.push_back(y_wp_car(i));
					}
